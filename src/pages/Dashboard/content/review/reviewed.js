...
import { actFetchReviewedAsync } from "../../../../store/post/actions";
function Reviewed() {
  const dispatch = useDispatch();
  const [page, setPage] = useState(0);
  const reviewed = useSelector((state) => state.Post.reviewed);
  useEffect(
    () => {
      dispatch(actFetchReviewedAsync());
    },
        ...
        <div className="col-lg-12 col-md-12 col-xs-12 traffic">
          <div className="dashboard-list-box margin-top-20 user-list">
            ...
            <ul>
              {reviewed && (
                <>
                  {reviewed.slice(page, page + 8).map((user, index) => (
